...
public class ConsultaJpaController implements Serializable {
    ...
    public List<Persona> getPacientes(Persona persona) {
        EntityManager em = getEntityManager();
        String jpql = "SELECT c.paciente FROM Consulta c WHERE c.medico = " + persona + "";
        Query query = em.createQuery(jpql);

        try {
            List<Persona> pacientes = query.getResultList();
            return pacientes;
        } finally {
            em.close();
        }
